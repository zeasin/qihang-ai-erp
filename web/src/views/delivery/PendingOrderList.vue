  ...
  <div class="page-card">
    ...
    <el-table :data="list" stripe border size="small" v-loading="loading">
          ...
          <div v-if="row.itemList && row.itemList.length" style="display:flex;flex-direction:column;gap:2px">
            <div v-for="(item, idx) in row.itemList.slice(0,2)" :key="idx" style="display:flex;align-items:flex-start;gap:8px;padding:4px 0;font-size:13px">
              <el-image :src="item.goodsImg || '/logo.png'" style="width:36px;height:36px;border-radius:4px;flex-shrink:0" fit="cover">
                <template #error><div style="width:36px;height:36px;background:#f5f5f5;display:flex;align-items:center;justify-content:center;font-size:12px;color:#ccc">图</div></template>
              </el-image>
            ...
            <span v-if="row.itemList.length > 2" style="color:#409eff;font-size:12px;cursor:pointer" @click="showDetail(row)">...共 {{ row.itemList.length }} 件商品</span>
          </div>
      ...
      <el-table-column label="操作" width="120" align="center" fixed="right">
        <template #default="{ row }">
          <el-button size="small" type="success" @click="openDelivery(row)">发货</el-button>
        ...
        <el-form-item label="发货商品">
          <el-table :data="deliveryItems" border size="small" max-height="300">
            ...
            <el-table-column label="库存" width="70" align="center">
              <template #default="{ row }">
                <span :style="{ color: (row._stockQty ?? 0) < row.quantity ? 'red' : '#67c23a' }">{{ row._stockQty ?? '-' }}</span>
              </template>
            </el-table-column>
...
const query = reactive({ orderNum: '', receiverName: '' })
...
function statusType(s: number) { const m: Record<number,string>={0:'warning',1:'danger',2:'primary',3:'success',11:'info'}; return m[s]||'info' }
function statusLabel(s: number) { const m: Record<number,string>={0:'待付款',1:'待发货',2:'已发货',3:'已完成',11:'已取消'}; return m[s]||'未知' }

function search() { pageNum.value = 1; fetchData() }
function resetQuery() { Object.assign(query, { orderNum: '', receiverName: '' }); search() }

async function fetchData() {
  loading.value = true
  try {
    const res: any = await request.get(api.orderList, {
      params: { pageNum: pageNum.value, pageSize: pageSize.value, ...query, orderStatus: 1 }
    })
...
function whChange(id: number) {
  ...
  deliveryItems.value.forEach((item: any) => {
    item._stockQty = 0; item._maxQty = item.quantity
...
function openDelivery(row: any) {
  ...
  deliveryForm.warehouseId = undefined; deliveryForm.warehouseName = ''
  deliveryForm.logisticsCompany = ''; deliveryForm.logisticsNo = ''; deliveryForm.remark = ''
  ...
  deliveryItems.value = (row.itemList || []).map((i: any) => ({
    orderItemId: i.id, goodsId: i.goodsId, goodsSkuId: i.goodsSkuId,
    goodsTitle: i.goodsTitle, goodsImg: i.goodsImg, goodsSpec: i.goodsSpec,
    skuNum: i.skuNum, quantity: i.quantity, _maxQty: i.quantity,
  }))
...
async function submitDelivery() {
  ...
  try {
    ...
    if (res.code === 200) {
      const deliveryId = res.data
      await request.post(api.deliveryConfirm(deliveryId))
      ElMessage.success('发货成功'); deliveryVisible.value = false; fetchData()
