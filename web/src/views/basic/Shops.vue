...
async function fetch() { const r:any = await request.get(api.shopList); list.value = r.data || [] }
async function fetchPlats() { const r:any = await request.get(api.platformList); platforms.value = r.data || [] }
...
async function save() { saving.value=true; try { await request.post(api.shopSave, dlg.form); ElMessage.success(dlg.isNew?'已创建':'已更新'); dlg.visible=false; await fetch() } catch(e:any) { ElMessage.error(e.message) } finally { saving.value=false } }
async function deleteRow(row:any) { try { await ElMessageBox.confirm('确定删除？','确认'); await request.delete(api.shopDelete(row.id)); ElMessage.success('已删除'); await fetch() } catch {} }
