  ...
  <div class="basic-layout">
    <el-container style="height: 100vh">
      <el-header class="basic-header">
        ...
        <span class="basic-title">📋 订单管理</span>
      </el-header>
      <el-container>
...
onMounted(async () => {
  try {
    ...
    const tree = res.data || []
    const orderMenu = tree.find((m: any) => m.path === '/order')
    if (orderMenu?.children) {
      childMenus.value = orderMenu.children.filter((c: any) => c.menuType === 'C' && c.visible !== '1')
    }
  } catch { /* ignore */ }
})
</script>
